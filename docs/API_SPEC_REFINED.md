...
> **날짜/시간 포맷:** ISO-8601(`Instant` 직렬화). 예) `2026-05-07T14:23:11.123Z`

---

## 1. 공통 응답 포맷

모든 REST 응답은 `ApiResponse<T>` envelope 으로 직렬화됩니다(TTS 바이너리 응답 제외). `@JsonInclude(NON_NULL)` 이 적용되어 있어 `null` 필드는 직렬화에서 제외됩니다.
...
### 1.3 nullable 필드 표기 규약

본 문서의 스키마 표(§5)에서 타입 컬럼의 `?` 접미사는 **nullable** 을 의미합니다. 예: `string?`, `Long?`, `Double?`. nullable 필드가 실제로 null 인 경우 `@JsonInclude(NON_NULL)` 정책에 따라 응답 JSON 에서 키 자체가 생략됩니다. `?` 가 없는 필드는 응답에 항상 존재함을 나타냅니다.

---

## 2. 인증 & 보안 모델

- **세션 정책:** `STATELESS`. 서버는 세션을 보관하지 않으며 모든 요청은 JWT 로 검증.
...
모두 공개. 응답 envelope 은 `ApiResponse<T>`. 회원가입은 `201 Created` 로 반환.
...
```json
{
  ...
  "data": {
    ...
    "user": {
      ...
      "streak": 0,
      "exp": 0,
      "createdAt": "2026-05-07T14:23:11.123Z"
    }
  }
}
```
...
| `recordingIds` | Long[] | NotEmpty | 누적 녹음 ID 목록(직전 `POST /api/recordings` 응답의 `id`) |
...
```json
// 응답 예시 — 모든 단계 만점이라 weakPhoneme 키가 omit 된 경우
{
  "success": true,
  "data": {
    "id": 31,
    ...
    "accuracy": 100.0,
    "practiceWord": "rabbit",
    "guidanceKr": "전반적으로 안정적인 발음이지만 더 연습하면 좋아요. 아래 단어로 다시 한 번 연습해 보세요.",
    "errors": [],
    "createdAt": "2026-05-07T14:23:11.123Z"
  }
}
...
| 파트 이름 | 타입 | 필수 | 설명 |
| --- | --- | --- | --- |
| `audio` | file | ✅ | 사용자 음성(WAV/등). 서버에서 WAV 로 변환 후 모델 서버 호출 |
...
```json
{
  ...
  "data": {
    ...
    "createdAt": "2026-05-07T13:08:21.450Z"
  }
}
```
...
```json
{
  ...
  "data": {
    "id": 142,
    "sessionId": 7,
    "sessionSentenceId": 41,
    "durationSec": 6.83,
    "perceived": ["aɪ", "l", "aɪ", "k"],
    "canonical": ["aɪ", "l", "aɪ", "k"],
    "peakSoftmax": [0.94, 0.88, 0.91, 0.86],
    "stepScore": 92.5,
    "guidanceKr": "발음이 자연스럽습니다.",
    "errors": [],
    "wrongWords": [],
...
각 응답에서 **해당 모드에 사용되지 않은 컨텍스트 키는 `@JsonInclude(NON_NULL)` 정책으로 omit** 됩니다 (예: script-flow 응답에 `sessionId` / `sessionSentenceId` 없음).
...
- 목적: 부분 수정. `null` 필드는 갱신하지 않음.
- 요청 본문: `SessionUpdateRequest`

| 필드 | 타입 | 제약 | 설명 |
| --- | --- | --- | --- |
| `title` | string? | ≤100자 | 제목 갱신 |
...
```json
// 응답 예시 — INTRO step 은 targetText 키가 omit 됨
{
  "success": true,
  "data": {
    "id": 12,
    "title": "Coffee Shop",
    "content": "Welcome to the coffee shop. ...",
    "difficulty": "EASY",
    "isPreset": true,
    "steps": [
      {
        ...
        "orderIndex": 0,
        "kind": "INTRO",
        "prompt": "오늘은 카페에서 주문하는 상황을 연습해요."
      },
      {
        "id": 72,
        "orderIndex": 1,
        "kind": "RECORD",
        "prompt": "아래 문장을 따라 읽어보세요.",
...
- 응답: `200 OK`, `Content-Type: audio/mpeg`, body 는 MP3 바이트.
- 주요 에러: `INVALID_REQUEST`(검증 실패), `MODEL_SERVER_UNAVAILABLE`, `MODEL_SERVER_ERROR`.

> 에러 시에는 `ApiResponse<Void>` JSON 으로 응답합니다(컨텐츠 협상 실패가 아닌 한). 클라이언트는 `Content-Type` 을 보고 분기하세요.
...
| `wrongWords` | `WrongWord[]` | LLM 이 짚어 준 단어(없으면 `[]`) |
...
| `weakPhoneme` | string? | 가장 약한 음소. 모든 녹음에 누적 오류가 없는 경우(모두 만점) 본 키가 응답에서 omit 됩니다 (`@JsonInclude(NON_NULL)`). |
| `practiceWord` | string | 권장 연습 단어. 시드 챕터 단어 → LLM 추천 → 음소 매핑 → 기본값(yaml `app.feedback.default-practice-word`) 순의 fallback 체인이 항상 채우므로 non-null 으로 보장됩니다. |
| `guidanceKr` | string | 한 줄 한글 가이드. 기본 LLM provider (`rule-based`) 가 모든 경로에서 non-null/non-empty 메시지를 빌드하므로 non-null 으로 보장됩니다. |
| `errors` | `PhonemeErrorResponse[]` | 누적 오답 음소 |
| `createdAt` | Instant | 생성 시각 |

> `practiceWord` / `guidanceKr` 의 non-null 보장은 본 명세서가 응답 contract 의 일부로 선언합니다 — 백엔드 구현은 fallback 체인을 유지해 이 보장을 깨지 않아야 합니다.
...
| `title` | string | 학습 단위 제목 |
| `accuracy` | double | 정확도 |
| `weakPhoneme` | string? | 약점 음소. 누적 오류가 없는 경우 본 키가 응답에서 omit 됩니다 (`@JsonInclude(NON_NULL)`). |
| `createdAt` | Instant | 생성 시각 |
...
| `targetText` | string? | RECORD 단계의 발음 대상 영어 문장. INTRO 단계에서는 본 키가 응답에서 omit 됩니다 (`@JsonInclude(NON_NULL)`). |

...
- 의존: `UserRepository`, `AppProperties`(`stats.zone` — streak 가산 시 ZoneId source-of-truth, `StatsService.attendance` 와 공통).
- 호출하는 entity 도메인 메서드: `User.changeNickname(...)`, `User.recordCompletion(expReward, ZoneId)` — ZoneId 는 `ZoneId.of(props.stats().zone())` 으로 주입 (§부록 D 의 streak/attendance 공통 zone 정책. ENTITIES_REFINED §2.1 과 본 명세서 §3.4.1 attendance 가 동일 ZoneId 를 사용해야 day=N 경계에서 streak 와 attendance 가 합치).
- ErrorCode: `USER_NOT_FOUND`, `VALIDATION_FAILED`.
- `awardCompletionRewards` 는 FeedbackService 가 atomic UPDATE 성공 후에만 호출(§5). 본문은 `User user = userRepository.findById(userId).orElseThrow(USER_NOT_FOUND)` → `user.recordCompletion(expReward, ZoneId.of(props.stats().zone()))` → 갱신된 `User` 반환. ZoneId 가 `StatsService.attendance` 와 동일 source 에서 결정되므로 KST 자정 경계에서도 streak (`UserResponse.streak`) 와 attendance (`StatsResponse.attendance.days[d]`) 가 항상 일치한다 (Codex finding F13 회귀 방지).
...
| streak ↔ attendance KST 자정 일치 (KST 23:59 에 complete → `/api/feedback/{id}/complete` 응답의 `UserResponse.streak` +1 + 같은 요청 직후 `/api/stats/me` 의 `attendance.days[N]` 키 존재; KST 다음날 00:30 에 별도 complete → `streak` +1 + `attendance.days[N+1]` 키 존재. DB 세션 tz / JVM tz 가 UTC 여도 두 응답이 같은 day 를 가리킴 — `MemberService.awardCompletionRewards` 와 `StatsService.attendance` 가 동일 `app.stats.zone` 을 ZoneId source 로 공유하기 때문. Codex finding F13 회귀 방지) | service | `@SpringBootTest` + Clock/Instant 주입 + complete 호출 직후 `/members/me` 와 `/stats/me` 응답 비교 |
...
- **streak / attendance 공통 ZoneId** — `User.streak` 가산 (`MemberService.awardCompletionRewards` → `User.recordCompletion`) 과 attendance 캘린더 (`StatsService.attendance`) 는 반드시 같은 `app.stats.zone` 을 사용한다. 두 곳이 다른 ZoneId 를 쓰면 KST 자정 경계의 완료가 streak 에는 day=N+1 로 (가산), attendance 에는 day=N 로 (집계) 분류되어 `/api/feedback/{id}/complete` 응답의 streak 과 `/api/stats/me` 의 attendance 가 silent 하게 어긋남. `AppProperties.Stats(String zone)` (yaml `app.stats.zone: Asia/Seoul`) 이 양쪽의 단일 source.

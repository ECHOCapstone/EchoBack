# IMPLEMENTATION PLAN (ARCHIVED — Phase 0~6 완료)

> **🗄 Archived 2026-05-13:** 그린필드 → Phase 6 (Hardening) 완료 시점에 `docs/IMPLEMENTATION_PLAN.md` 에서 본 위치로 이동. 후속 FRONT_API_SPEC contract 정렬 작업 (rev6 D1-D3 + Codex 2026-05-13 D4-D12) 은 `docs/IMPLEMENTATION_PLAN.md` 가 다룬다. 본문에서 자기 자신을 `docs/IMPLEMENTATION_PLAN.md` 로 가리키는 줄(예: §12.4) 은 archive 이전 시점 경로이므로 수정하지 않는다.

...
create table pronunciation_feedbacks (accuracy float(53) not null, completed bit not null, completed_at datetime(6), created_at datetime(6) not null, id bigint not null auto_increment, script_id bigint, session_id bigint, user_id bigint not null, weak_phoneme varchar(32), practice_word varchar(100), task_title varchar(200), title varchar(200) not null, comprehensive_json TEXT, guidance_kr TEXT, script_text TEXT, primary key (id), constraint ck_pronunciation_feedbacks_script_session_xor check ((script_id IS NULL AND session_id IS NULL) OR ((script_id IS NULL) <> (session_id IS NULL)))) engine=InnoDB;
create table recordings (duration_sec float(53), step_score float(53), created_at datetime(6) not null, id bigint not null auto_increment, script_id bigint, session_id bigint, session_sentence_id bigint, step_id bigint, user_id bigint not null, audio_path varchar(500) not null, canonical TEXT, errors_json TEXT, guidance_kr TEXT, peak_softmax TEXT, perceived TEXT, target_text_snapshot TEXT, wrong_words_json TEXT, primary key (id), constraint ck_recordings_script_session_xor check ((script_id IS NULL AND session_id IS NULL) OR ((script_id IS NULL) <> (session_id IS NULL))), constraint ck_recordings_step_requires_script check (step_id IS NULL OR script_id IS NOT NULL), constraint ck_recordings_sentence_requires_session check (session_sentence_id IS NULL OR session_id IS NOT NULL)) engine=InnoDB;
